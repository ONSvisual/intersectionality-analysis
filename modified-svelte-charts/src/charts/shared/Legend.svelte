...
{#if Array.isArray(_domain) && Array.isArray(colors)}
  <ul class="legend" aria-hidden="true">
    {#each _domain as label, i}
      <li>
        <div
          class={i==0 ? "round bullet" : "diamond bullet"}
    ...
    <li>
      ...
      {index >3 ? "Previous model" :"Raw estimates"}
    </li>
    {/if}
  </ul>
{/if}
...
<style>
  ...
  ul.legend li div, ul.legend li svg{
    margin-right:5px;
  }
  ul.legend li + li {
    margin-left: 25px;
