...
<script>
  ...
  import { tweened } from 'svelte/motion';
	...
	import GhostPlot from './shared/GhostPlot.svelte';
	import HighlightSnake from './shared/HighlightSnake.svelte';
	import RefLine from './shared/RefLine.svelte';
	...
	$: if(ghosts) ghostsGrouped = groupData(ghosts,yDomain,yKey)
</script>
...
<slot name="options"/>
{#if legend && _zDomain}
  <Legend domain={_zDomain} {colors} horizontal={false} line={mode == 'barcode'} comparison={mode == 'comparison'} {ghosts} {index}/> 
  <slot name="legend"/>
{/if}
<div class="chart-container" style="height: {typeof height == 'number' ? `${height}px` : height ?  height : yDomain ? `${padding.top + padding.bottom + (barHeight * yDomain.length)}px` : "300px" }" aria-hidden="true">
	...
	>
	  ...
	  <slot name="back"/>
		...
		{#if yAxis}
			  <AxisY gridlines={true} trimGridlines={true} prefix={yPrefix} suffix={ySuffix} {textColor} {tickColor} {highlighted} tickDashed={true} {mobile} {other}/>
		{/if}
			<HighlightSnake {snake}/>
			<HighlightSnake snake={snake2}/>
		{#if ghosts}
			<GhostPlot ghosts={ghostsGrouped} colours={["#78b2e2","#fbb0b4"]} {highlighted}/>		
	  	{/if}

		{#if refvalue}
			<RefLine {refvalue}/>
		{/if}
		<DotPlot {select} {selected} {hover} {hovered} {highlighted} on:hover on:select {overlayFill} {other}/>
